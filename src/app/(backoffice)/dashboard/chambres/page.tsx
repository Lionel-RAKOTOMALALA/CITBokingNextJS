...
import AddChambreModal from "@/components/chambres/AddChambreModal";
import EditChambreModal from "@/components/chambres/EditChambreModal";
...
export default function ChambresSection() {
  const { data, isLoading, refetch } = trpc.chambre.list.useQuery();
  ...
  const createMutation = trpc.chambre.create.useMutation({
    onSuccess: () => utils.chambre.list.invalidate(),
  });
  const updateMutation = trpc.chambre.update.useMutation({
    onSuccess: () => utils.chambre.list.invalidate(),
  });
  const deleteMutation = trpc.chambre.delete.useMutation({
    onSuccess: () => {
      utils.chambre.list.invalidate();
      setDeleteId(null);
    },
  ...
  const filtered = data?.filter((c) =>
    ...
    c.hebergement?.nom?.toLowerCase().includes(search.toLowerCase()) ||
    c.typeChambre?.nom?.toLowerCase().includes(search.toLowerCase())
  ) ?? [];
  ...
  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="text-gray-400 text-xs mb-1">Chambres • Listes</div>
          <h1 className="text-xl font-semibold text-white">Liste des chambres</h1>
        </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-gradient-to-r from-gray-700 to-gray-750 border-b border-gray-600">
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-300 uppercase tracking-wider">Numéro</th>
            ...
            <tbody className="divide-y divide-gray-700">
              {isLoading ? (
                ...
                <tr><td colSpan={5} className="text-center py-8 text-gray-400">Aucune chambre trouvée.</td></tr>
              ) : filtered.map((c) => (
                <tr key={c.id} className="hover:bg-gray-750 transition-colors duration-150">
                  <td className="px-4 py-3">
                    <span className="text-sm font-medium text-white">{c.numero}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-gray-300">{c.hebergement?.nom || <span className="italic text-gray-500">-</span>}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-sm text-gray-300">{c.typeChambre?.nom || <span className="italic text-gray-500">-</span>}</span>
                  </td>
                  <td className="px-4 py-3">
                    {c.disponible ? (
      ...
      <div className="flex items-center justify-between mt-4 text-xs text-gray-400">
        <span>
          {filtered.length} chambre{filtered.length > 1 ? "s" : ""} au total
        </span>
      ...
      <Modal open={!!deleteId} onClose={() => setDeleteId(null)}>
        <div className="p-8 flex flex-col items-center">
          ...
          <div className="text-slate-300 mb-6 text-center">
            Êtes-vous sûr de vouloir supprimer cette chambre ? Cette action est <b>irréversible</b>.
          </div>
